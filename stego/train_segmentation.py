...
def IIC(p1, p2, lamb=1.5, eps=.000001):
    C = p1.shape[1]
    P = torch.exp(p1.unsqueeze(2) + p2.unsqueeze(1)).sum(dim=0)
    P = ((P + P.t()) / 2) / P.sum()
    P = torch.clamp_min(P, eps)
    Pi = P.sum(dim=1).view(C, 1).expand(C, C)
    Pj = P.sum(dim=0).view(1, C).expand(C, C)
    return (P * (lamb * torch.log(Pi) + lamb * torch.log(Pj) - torch.log(P))).sum()
...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        elif cfg.arch == "iic-baseline":
            from iic_code import IICBaseline
            self.net = IICBaseline(cfg.dataset_name, True)
        elif cfg.arch == "swin":
