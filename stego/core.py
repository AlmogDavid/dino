...
#from voc import CustomVOC
from torch.utils.data._utils.collate import np_str_obj_array_pattern, default_collate_err_msg_format, string_classes
int_classes = int
...
class UnsupervisedMetrics(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        with torch.no_grad():
            ...
            actual = actual[mask]
            preds = preds.reshape(-1)[mask]
            self.stats += torch.bincount(
                (self.n_classes + self.extra_clusters) * actual + preds,
                minlength=self.n_classes * (self.n_classes + self.extra_clusters)) \
                .reshape(self.n_classes, self.n_classes + self.extra_clusters).t()
...
class Potsdam(Dataset):
    def __init__(self, root, image_set, transform, target_transform, coarse_labels):
        ...
        assert self.split in ["train", "test", "train+test", "unlabelled_train"]
        split_files = {
            "train": ["labelled_train.txt"],
            "test": ["labelled_test.txt"],
            "unlabelled_train": ["unlabelled_train.txt"],
            "train+test": ["labelled_train.txt", "labelled_test.txt"]
        }
    ...
    def __getitem__(self, index):
        ...
        label = torch.from_numpy(label).unsqueeze(-1).permute(2, 0, 1)
