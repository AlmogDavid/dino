...
class DinoFeaturizer(nn.Module):
    ...
    def make_clusterer(self, in_channels):
        ...
        # LambdaLayer(DinoFeaturizer._helper))
...
class SwinFeaturizer(nn.Module):
    ...
    def __init__(self, dim, cfg):
        ...
        self.feat_type = self.cfg.dino_feat_type
        arch = self.cfg.model_type
        self.model = swins.__dict__[arch](
            patch_size=patch_size)
        for p in self.model.parameters():
            p.requires_grad = False
        self.model.eval().cuda()
        ...
        if cfg.pretrained_weights is None:
            raise RuntimeError("Must give path to weights")
        state_dict = torch.load(cfg.pretrained_weights, map_location="cpu")
        state_dict = state_dict["teacher"]
        # remove `module.` prefix
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        ...
        msg = self.model.load_state_dict(state_dict, strict=False)
        print('Pretrained weights found at {} and loaded with msg: {}'.format(cfg.pretrained_weights, msg))
        self.n_feats = 1344 # todo: NEED TO CHANGE THIS VALUE
        self.cluster1 = self.make_clusterer(self.n_feats)
        self.proj_type = cfg.projection_type
        if self.proj_type == "nonlinear":
            self.cluster2 = self.make_nonlinear_clusterer(self.n_feats)
    ...
    def forward(self, img, n=1, return_class_feat=False):
        ...
        with torch.no_grad():
            ...
            # get selected layer activations
            image_feat = self.model.get_intermediate_feat(img, n=n)

            # feat_h = img.shape[2] // 8  # TODO: we resize to 8x8 patch size
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def __init__(self, cfg, ):
        ...
        if self.cfg.use_recalibrator:
            self.inter_cal = Recalibrator()
            self.intra_cal = Recalibrator()
            self.neg_cal = Recalibrator()
            cal_state_dict = torch.load(join(cfg.output_root,
                                             "checkpoints/loss_exp_3/cocostuff27_exp5_date_Sep07_17-58-44.pt"))
            self.intra_cal.load_state_dict(cal_state_dict["intra"], strict=True)
            self.inter_cal.load_state_dict(cal_state_dict["inter"], strict=True)
            self.neg_cal.load_state_dict(cal_state_dict["neg"], strict=True)
        else:
            self.inter_cal = None
            self.intra_cal = None
            self.neg_cal = None
    ...
    def helper(self, f1, f2, c1, c2, cal, shift):
        with torch.no_grad():
        ...
        if self.cfg.use_recalibrator:
            fd = cal.forward_shaped(fd)


        if self.cfg.stabalize:
            loss = - cd.clamp(0, .8) * (fd - shift)
            #loss += 0 - torch.clamp_max(cd, 0)
            #loss += torch.clamp_min(cd, .8) - .8
        else:
    ...
    def forward(self,
        ...
        pos_intra_loss, pos_intra_cd = self.helper(
            feats, feats, code, code, self.intra_cal, self.cfg.pos_intra_shift)
        pos_inter_loss, pos_inter_cd = self.helper(
            feats, feats_pos, code, code_pos, self.inter_cal, self.cfg.pos_inter_shift)
        ...
        for i in range(self.cfg.neg_samples):
            ...
            neg_inter_loss, neg_inter_cd = self.helper(
                feats, feats_neg, code, code_neg, self.neg_cal, self.cfg.neg_inter_shift)
...
class Recalibrator(nn.Module):
    def __init__(self):
        ...
        self.linear.weight.data.copy_(torch.tensor(1.0))
        self.linear.bias.data.copy_(torch.tensor(0.0))

    def forward_shaped(self, x):
        return self.forward(x.reshape(-1, 1), True).reshape(*x.shape)

    def forward(self, x, return_prob):
        # x = 1 / (1 + torch.exp(-x))
        x = (x + 1) / 2
        logit = -torch.log((1 / x.clamp(.001, .999)) - 1)
        new_logit = self.linear(logit) + self.nonlinear(logit)
        if return_prob:
            return torch.sigmoid(new_logit)
        else:
            return new_logit
