...
def get_relative_position_index(wH, wW):
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # get pair-wise relative position index for each token inside the window
        relative_position_index = get_relative_position_index(self.window_size[0], self.window_size[1])  # Wh*Ww, Wh*Ww
    ...
    def forward(self, x, mask=None):
        ...
        # create index for given window
        window_size = int(np.sqrt(N))
        relative_position_index = self.relative_position_index.clone()
        orig_window_size = int(np.sqrt(relative_position_index.shape[0]))
        index_window = torch.arange(orig_window_size ** 2).view(orig_window_size, orig_window_size)[:window_size, :window_size].flatten()
        # print(orig_window_size,window_size)
        # print(relative_position_index.shape,index_window.min(), index_window.max())
        relative_position_index = relative_position_index[index_window][:, index_window]
        ...
        relative_position_bias = relative_position_bias.view(window_size * window_size,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dim=96, depths=(2, 2, 6, 2),
        ...
        self.num_features_last = num_features[-1]
        self.mlp_ratio = mlp_ratio
    ...
    def forward(self,
        ...
        if f_type in ['segment' , 'pool_features', 'flat_features']:
            ...
            for end_idx in idx_crops:
                _out = self.forward_segmentation(torch.cat(x[start_idx: end_idx]))
                if start_idx == 0:
                    output = [_out]
                else:
                    output.append(_out)
